<section
    id="stats"
    class="relative py-16 lg:py-24 bg-ipn-gray-light" {{-- Fondo claro --}}
>
    {{-- Eliminado el overlay oscuro que estaba antes --}}
    ...
    <div class="relative container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {{-- Subtítulo ajustado para fondo claro --}}
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            <div
                ...
                {{-- Tarjeta adaptada a fondo claro --}}
                class="bg-white rounded-xl p-6 shadow-md border border-gray-200 flex flex-col items-center space-y-3 text-center transition-shadow hover:shadow-lg"
            >
                {{-- Icono (color guinda) --}}
                <div class="flex-shrink-0 w-12 h-12 text-ipn-guinda">
                    <svg fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 6.042A8.967 8.967 0 0 0 6 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 0 1 6 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 0 1 6-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0 0 18 18a8.967 8.967 0 0 0-6 2.292m0-14.25v14.25" />
                    </svg>
                </div>
                {{-- Contador (color guinda) --}}
                <div class="text-4xl lg:text-5xl font-teko font-bold text-ipn-guinda" x-text="display"></div>
                {{-- Etiqueta (color oscuro) --}}
                <p class="text-lg font-medium text-ipn-gray-dark">Recursos Disponibles</p>
            </div>
            ...
            >
                ...
                <div class="text-4xl lg:text-5xl font-teko font-bold text-ipn-guinda" x-text="display"></div>
                 {{-- Etiqueta (color oscuro) --}}
                <p class="text-lg font-medium text-ipn-gray-dark">Usuarios Activos</p>
            </div>

            {{-- Stat 3: Bibliotecas Conectadas --}}
            <div
                x-data="countUp(20,'+')" x-intersect.once="start()"
                class="bg-white rounded-xl p-6 shadow-md border border-gray-200 flex flex-col items-center space-y-3 text-center transition-shadow hover:shadow-lg"
            >
                {{-- Icono (color guinda) --}}
                <div class="flex-shrink-0 w-12 h-12 text-ipn-guinda">
                ...
                <div class="text-4xl lg:text-5xl font-teko font-bold text-ipn-guinda" x-text="display"></div>
                 {{-- Etiqueta (color oscuro) --}}
                <p class="text-lg font-medium text-ipn-gray-dark">Bibliotecas Conectadas</p>
            </div>
            ...
            >
                ...
                <div class="text-4xl lg:text-5xl font-teko font-bold text-ipn-guinda" x-text="display"></div>
                 {{-- Etiqueta (color oscuro) --}}
                <p class="text-lg font-medium text-ipn-gray-dark">Foros y Grupos</p>
            </div>

        </div> {{-- Fin del grid --}}
    </div>
    {{-- Script de Alpine.js para el contador (Asegúrate que Alpine esté cargado en tu layout) --}}
    <script>
        function countUp(target, suffix = '') {
          return {
            count: 0,
            display: '0' + suffix, // Mostrar sufijo inicial
            start() {
              if (this.count !== 0) return; // Evitar reiniciar si ya contó
              ...
              const timer = setInterval(() => {
                ...
                // Formatear número con separador de miles para México
                this.display = new Intl.NumberFormat('es-MX', { maximumFractionDigits: 0 }).format(Math.round(this.count)) + suffix;

                if (currentFrame === totalFrames) {
                  this.count = target; // Asegurar valor final exacto
                  this.display = new Intl.NumberFormat('es-MX', { maximumFractionDigits: 0 }).format(target) + suffix;
                  clearInterval(timer);
                }
              }, frameRate);
            },
